...
func main() {
	...
	//创建EC_P256非对称密钥，用途为SIGN/VERIFY
	keyId, err := CreateKey(client, "EC_P256", "SIGN/VERIFY")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	//KMS签名 本地验签
	err = ecP256SignVerify(client, keyId, keyVersionId, message, algorithm)
	if err != nil {
		fmt.Println(err)
	}
}
...
func ecP256SignVerify(client *kms.Client, keyId, keyVersionId, message, algorithm string) error {
	signature, err := kms_api_samples.AsymmetricSign(client, keyId, keyVersionId, message, algorithm)
	if err != nil {
		return errors.New(fmt.Sprintf("AsymmetricSign error:%+v", err))
	}
	err = kms_api_samples.EcP256Verify(client, keyId, keyVersionId, message, signature)
	if err != nil {
