...
func main() {
	...
	//创建EC_P256K非对称密钥，用途为SIGN/VERIFY
	keyId, err := CreateKey(client, "EC_P256K", "SIGN/VERIFY")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	//EC_P256K非对称密钥签名、验签
	//KMS签名 KMS验签
	err = asymmetricSignVerify(client, keyId, keyVersionId, message, "ECDSA_SHA_256")
	if err != nil {
		fmt.Println(err)
	}

	//KMS签名 本地验签
	err = ecP256KSignVerify(client, keyId, keyVersionId, message, "ECDSA_SHA_256")
	if err != nil {
		fmt.Println(err)
	}
}
...
func ecP256KSignVerify(client *kms.Client, keyId, keyVersionId, message, algorithm string) error {
	signature, err := kms_api_samples.AsymmetricSign(client, keyId, keyVersionId, message, algorithm)
	if err != nil {
		return errors.New(fmt.Sprintf("AsymmetricSign error:%+v", err))
	}
	err = kms_api_samples.EcP256KVerify(client, keyId, keyVersionId, message, signature)
	if err != nil {
		return errors.New(fmt.Sprintf("EcVerify error:%+v", err))
	}
	return nil
}
